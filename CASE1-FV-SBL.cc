...
卒業研究で
文献
T. Buchgraber, D. Shutin, and H.V. Poor. A sliding-window online fast variational sparse bayesian learning algorithm. 
In Acoustics, Speech and Signal Processing (ICASSP), 2011 IEEE International Conference on, pages 2128–2131, May 2011.
で導入されているアルゴリズムを実装し，比較対象とした。
この文献で導入されているSW-FV-SBLのモデルのsliding-windowブロックの長さの上限を無限大に設定することによって，
FV-SBLの逐次学習版として機能する。
＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿
#endif

//g++ -Wall -std=c++11 -I/usr/local/include CASE1-FV-SBL.cc
//valgrind --leak-check=full --show-leak-kinds=all ./a.out < CASE1-dataset.txt
int total_num = 0;
int dimension = 0;
FILE *gp;
...
int main(){
    ...
    for (int i = 0; i < 100; i++){
        ...
        prediction_error[0] += 100.0*abs(sin(x_vector(0,0)) - k_d_online_predicton(x_vector, basic_func_num, basic_func_check, mean,input_data_x));
    }
    ...
    for (tau = 1; tau <= total_num-1; tau++){
        ...
        for (int i = 0; i <= dimension-1; i++){
            input_data_x(input_data_num-1,i) = input_data_container(input_data_num-1,i);
        }
        
        target_t(input_data_num-1,0) = target_t_container(input_data_num-1,0);
    ...
    if (k_d_add_Check_new_l(input_data_num-1, noise_precision ,basic_func_num, input_data_num, design_matrix, alpha,
         input_data_container, input_data_x, target_t) > 0){
         double new_alpha = k_d_add_Check_new_l(input_data_num-1, noise_precision ,basic_func_num, input_data_num, design_matrix, alpha,
         input_data_container, input_data_x, target_t);
         basic_func_num++;
         basic_func_check(input_data_num-1,0) = 1;
         design_matrix = k_d_design_matrix_update(input_data_num, basic_func_num, basic_func_check, input_data_x);
    ...
    for (int i = 0; i < 100; i++){
        x_vector(0,0) = my_uniform_distribution(generator);
        prediction_error[tau] += 100.0*abs(sin(x_vector(0,0)) - k_d_online_predicton(x_vector, basic_func_num, basic_func_check, mean,input_data_x));
    }
    ...
    fprintf(gp, "plot '-' with lines linestyle 6 linetype 6 linewidth 1 dt '_.-.' title \"FV-SBL\"\n");
