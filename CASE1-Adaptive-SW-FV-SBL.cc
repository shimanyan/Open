...
で導入されているストリームデータに対するSegmentation Algorithmを組み合わせて
提案手法となるモデル"Adaptive-SW-FV-SBL"を定式化した。
データ源が時間の経過とともに変化しうる，すなわちConcept Driftが発生するストリームデータに対して
sliding-windowブロックの長さの最適化を自動で行ない，適応的な学習が可能となる。
＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿
#endif

//g++ -Wall -std=c++11 -I/usr/local/include CASE1-Adaptive-SW-FV-SBL.cc
...
int main(){
    ...
    for (int i = 0; i < 100; i++){
        ...
        prediction_error[0] += 100.0*abs(sin(x_vector(0,0)) - k_d_online_predicton_SW(x_vector, basic_func_num, basic_func_check, mean,input_data_x,input_data_container));
    }
    ...
    for (int i = 0; i < 100; i++){
        x_vector(0,0) = my_uniform_distribution(generator);
        prediction_error[tau] += 100.0*abs(sin(x_vector(0,0)) - k_d_online_predicton_SW(x_vector, basic_func_num, basic_func_check, mean,input_data_x,input_data_container));
    }
