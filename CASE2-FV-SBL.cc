...
卒論のCASE2の実験のための逐次FV-SBLのプログラム。
CASE2はSudden Concept Driftの発生を想定
___________________________________＿＿＿＿＿
#endif

//g++ -Wall -std=c++11 -I/usr/local/include CASE2-FV-SBL.cc
...
int main(){
    ...
    for (int i = 0; i < 100; i++){
        ...
        prediction_error[0] += 100.0*abs(sin(x_vector(0,0)) + offset - k_d_online_predicton(x_vector, basic_func_num, basic_func_check, mean,input_data_x));
    }
    ...
    for (int i = 0; i < 100; i++){
        x_vector(0,0) = my_uniform_distribution(generator);
        prediction_error[tau] += 100.0*abs(sin(x_vector(0,0)) + offset - k_d_online_predicton(x_vector, basic_func_num, basic_func_check, mean,input_data_x));
    }
